/** *************************************************************
****************************************************************
TVEG_GDS.CPP - object describing characteristics of vegetation
...
// cout << "vegh = " << pen.getVEGH() << " " << zh << " " << href[pdcmnt] << " " << kallom[pdcmnt] << " " << cabove << " " << leaf.carbon << " " << heartwood.carbon << " " << sapwood.carbon << endl;
...
//  cout << "dgppdleafc = " << dgppdleafc << " " <<  frdl << " " << fco2 << " " << temp << " " << fozone << " " << fh2o << " " << pen.getDFWS_DLEAFC() << " " << rgpp << " " << ringpp << endl;
...
// check to see if overall allocation is C-limited or N-limited
  if(nfeed == 1)
  {
//if(pdyr > 800)    {cout << "Nlimitation = " << leaf.allocn << " " << cnleaf[pdcmnt] << " " << leaf.allocc << endl;}
...
//if(pdyr > 800)       {cout << "nlimited = " << leaf.allocc << endl;}
    }
    else // c-limited
    {
...
//cout << "gpp = " << veggpp << " " << par << " " << pdayl << " " << ki[pdcmnt] << " " << cmax << " " << pndays << " " << kext << " " << kc[pdcmnt] << endl;
//if(pdyr > 722) { cout << "veggpp " << veggpp << " " << cmax << " " << cmaxgcmo << " " <<  frdl << " " << fco2 << " " << temp << " " << fozone << " " << fh2o << " " << lai << endl;}
//  cout << "frdl " << frdl << " " << kext << " " << kiw << " " << parw << " " <<  lai << endl;
...
//if(pdyr > 800) {cout << "dgppdleafc = " << dgppdleafc << " " << cmaxgcmo << " " << frad << " " << frdl << " " << fco2 << " " << temp << " " << fozone << " " << fh2o << " " << pen.getDFWS_DLEAFC() << endl;}
...
void Tveg45::setTEMP( const int& pdcmnt, const double& tair, const int& pdyr )
...
//  if( tair <= tmin[pdcmnt] || tair >= tmax[pdcmnt] || topt >= tmax[pdcmnt] )
  if( tair <= tmin[pdcmnt] || tair >= tmax[pdcmnt] || topt >= tmax[pdcmnt]-1.0 )
  {
    temp = ZERO;
  }
  else
...
//if(pdyr > 722) {    cout << "first temp = " << temp << " " << pow2 << " " << tair << " " << tmax[pdcmnt] << " " << tmin[pdcmnt] << " " << topt << endl;}
    temp /= pow((tmax[pdcmnt]-topt),pow1)*pow((topt - tmin[pdcmnt]),pow2);
//if(pdyr > 722) {cout << "second temp = " << temp << endl;}
  }
...
//    if(pdyr > 800) {cout << "ndiag = " << rdemandn << " " << rdemandc << " " << rlabilen << " " << rlabilec << " " << labile.carbon << " " << labile.nitrogen << " " << cnprod << endl;}
...
//        if(pdyr > 800) { cout << "GPP downregulated" << endl;}
...
//      soninput = 0.0;
    ...
    if ( rdemandn < ZERO ) 
      if( (rlabilec/rlabilen > rdemandc/fabs(rdemandn))
           && (labile.carbon/labile.nitrogen > cnprod) )
    {
    {
        fgppdownreg = (rdemandc/fabs(rdemandn))/(rlabilec/rlabilen);
    } 
    } 
...
//    if(initFlag == 1) {cout << "npp = " << npp << " " << gpp << " " << plant.rm << " " << plant.rg << endl;}
