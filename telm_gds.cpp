/* **************************************************************
*****************************************************************
TELM_GDS.CPP - Runs TEM for a single grid cell
...
//      BSF CO2 Fix to allow transient CO2
...
//    smoisteq[(dyr%de_nyears)] = tem.getY(tem.I_SM);

    avlneq[(dyr%de_nyears)] = tem.soil.yravln;
    vegceq[(dyr%de_nyears)] = tem.veg.yrcarbon;
    soilceq[(dyr%de_nyears)] = tem.soil.yrorgc;
    vegneq[(dyr%de_nyears)] = tem.veg.yrnitrogen;
    soilneq[(dyr%de_nyears)] = tem.soil.yrorgn;
    smoisteq[(dyr%de_nyears)] = tem.soil.yrsmoist;
...
//  output[tem.I_INNPP][pdm] = tem.getY( tem.I_INNPP );
  output[tem.I_INNPP][pdm] = tem.veg.getINNPP();

//  output[tem.I_NPP][pdm] = tem.getY( tem.I_NPP );
  output[tem.I_NPP][pdm] = tem.veg.getNPP();

//  output[tem.I_GPR][pdm] = tem.getY( tem.I_GPR );
  output[tem.I_GPR][pdm] = tem.veg.getGPR();
  ...
  output[tem.I_NBP][pdm] = tem.getNBP();
  ...
  if( 0 == tem.ag.state )
  {
    ...
    output[tem.I_NATLAI][pdm] = tem.getY( tem.I_LAI );
...
//    output[tem.I_NATGPR][pdm] = tem.getY( tem.I_GPR );
  ...
  else
  {
    ...
    output[tem.I_CROPLAI][pdm] = tem.getY( tem.I_LAI );
...
//
  ...
  tem.ag.setFERTNCROP( nfertcrop[0][dyr] );
  tem.ag.setFERTNPASTURE( nfertpasture[0][dyr] );
  for( dm = 0; dm < CYCLE; ++dm )
  {
...
rflog1 << "final equilibration year = " << year << " " << tem.veg.cmnt << endl;
...
//        cout << "INGPP in telm = " << dm << " " << tempred.mon[dm] << endl;
    ...
    else if( predname.at( i ) == tem.predstr.at( tem.I_NBP ) )
    {
      for( dm = 0; dm < CYCLE; ++dm )
      {
        tempred.mon[dm] = output[tem.I_NBP][dm];
      }
    }
...
//cout << "entering updateTEMmonth" << endl;
      ...
      && ACCEPT == cohort[pichrt].tqc )
  {
    ...
    tem.ag.setFERTNCROP( nfertcrop[0][pdyr] );
    tem.ag.setFERTNPASTURE( nfertpasture[0][pdyr] );
...
//      tem.atms.getSNOWFALL() << " " << tem.atms.get//CO2() << " " << tem.atms.getAOT40() << " " <<
