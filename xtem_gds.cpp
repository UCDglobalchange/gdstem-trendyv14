/* *************************************************************
XTEM_GDS.CPP - Extrapolation version of the Terrestrial Ecosystem
               Model Version 4.5
****************************************************************
...
#include "temconsts_gds.hpp"   // Global constants
#include "tclmdat_gds.h"       // Clmdat45 class
#include "tco2dat_gds.h"       // CO2dat45 class
#include "elmnt_gds.h"         // Elmnt45 Class
#include "latdat_gds.h"        // Latdat45 class
#include "telm_gds.h"          // Telm45 Class
...
Nfertdat nfertcropdat[MAXRTIME];
Nfertdat nfertpasturedat[MAXRTIME];
#ifdef STORM
...
//  flog1 << endl << endl;
  ...
  elmnt.show( flog1,
      ...
      if(telmnt[0].clm.cldflag == 1)
        ...
        for( xdyr = 0; xdyr < (transtime); ++xdyr )
        {
          gisend = ws10dat[xdyr].getdel( ifws10 );
        }
        }
        else
        {
        ...
        if(telmnt[0].nfert.tnfertcropflag == 1)
        {
        for( xdyr = 0; xdyr < (transtime); ++xdyr )
        {
          gisend = nfertcropdat[xdyr].getdel( ifnfertcrop );
        }
        }
        else
        {
          gisend = nfertcropdat[0].getdel( ifnfertcrop );
        }
        if(telmnt[0].nfert.tnfertpastureflag == 1)
        {
        for( xdyr = 0; xdyr < (transtime); ++xdyr )
        {
          gisend = nfertpasturedat[xdyr].getdel( ifnfertpasture );
        }
        }
        else
        {
          gisend = nfertpasturedat[0].getdel( ifnfertpasture );
        }
...
#endif
          // gisend = ws10dat[0].getdel( ifws10 ); // commenting this out to make wind transient
...
void initializeCLMGridCell()
{
  ...
  string o3name;
  string ndepname;
  string nfertcropname;
  string nfertpasturename;
  ...
  // Open nfert_crop file
  //
  tempfname.str( "" );

  tempfname << telmnt[0].nfert.nfertcropfname
            << telmnt[0].nfert.nfertcropend;

  nfertcropname = tempfname.str();

  ifnfertcrop = fopen( nfertcropname.c_str(), "r" );
  if( !ifnfertcrop )
  {
    flog1 << endl << "Cannot open " << nfertcropname;
    flog1 << " for NFERT_CROP data input" << endl << endl;

    exit( -1 );
  }

  // Open nfert_pasture file
  //
  tempfname.str( "" );

  tempfname << telmnt[0].nfert.nfertpasturefname
            << telmnt[0].nfert.nfertpastureend;

  nfertpasturename = tempfname.str();

  ifnfertpasture = fopen( nfertpasturename.c_str(), "r" );
  if( !ifnfertpasture )
  {
    flog1 << endl << "Cannot open " << nfertpasturename;
    flog1 << " for NFERT_PASTURE data input" << endl << endl;
...
/// Climate inputs  
  ...
  if( telmnt[0].clm.tws10flag == 1 ) { flog1 << " transient ws10 dataset: " << telmnt[0].clm.iws10fname + telmnt[0].clm.iws10end << endl; }
  else { flog1 << " long-term average ws10 dataset: " << telmnt[0].clm.iws10fname << endl; }
...
//  Input name of Nfert crop file
//
  telmnt[0].nfert.tnfertcropflag = telmnt[0].tem.goxml.getXMLint( telmnt[0].tem.gofile, "gofile", "xnfertcropflag" );
  telmnt[0].nfert.nfertcropfname = telmnt[0].tem.goxml.getXMLstring( telmnt[0].tem.gofile, "gofile", "nfertcropfname" );
  telmnt[0].nfert.nfertcropend = telmnt[0].tem.goxml.getXMLstring( telmnt[0].tem.gofile, "gofile", "nfertcropend" );
  if( telmnt[0].nfert.tnfertcropflag == 1 ) { flog1 << " transient nfertcrop dataset: " << telmnt[0].nfert.nfertcropfname + telmnt[0].nfert.nfertcropend << endl; }
  else { flog1 << " long-term average nfertcrop dataset: " << telmnt[0].nfert.nfertcropfname << endl; }

//  Input name of Nfert pasture file
//
  telmnt[0].nfert.tnfertpastureflag = telmnt[0].tem.goxml.getXMLint( telmnt[0].tem.gofile, "gofile", "xnfertpastureflag" );
  telmnt[0].nfert.nfertpasturefname = telmnt[0].tem.goxml.getXMLstring( telmnt[0].tem.gofile, "gofile", "nfertpasturefname" );
...
//commenting out to make wind transient  
  ...
  for(i = 0; i < telmnt[0].natmspred; ++i )
  {
    ...
    string startval_str = std::to_string(startval);
...
void updateTCLMGridCell( const int& pdyr )
{
  ...
  if( 0 == pdyr )
  {
...
#ifdef DEBUGX
   cout << "girr in TCLMGridCell" << endl;
#endif
...
#ifdef DEBUGX
   cout << "if cloud logic in TCLMGridCell" << endl;
#endif
        telmnt[0].col = cldsdat[0].col;
        telmnt[0].row = cldsdat[0].row;
        SRADname = "CLDINESS";
#ifdef DEBUGX
   cout << "read in clouds in TCLMGridCell" << endl;
#endif
      }
      else
      {
        ...
        SRADname = "NIRR";
#ifdef DEBUGX
   cout << "read in nirr in TCLMGridCell" << endl;
#endif
      }
#ifdef DEBUGX
   cout << "clds/nirr in TCLMGridCell" << endl;
#endif
  ...
  #ifdef DEBUGX
   cout << "get cohort number data - if part" << endl;
   cout << "nirr col " << telmnt[0].col << endl;
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "prec col " << precdat[0].col << endl;
   cout << "prec row " << precdat[0].row << endl;
   cout << "prec row " << precdat[0].year << endl;
   cout << "year " << dyr << endl; 
#endif
   ...
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "trange col " << trangedat[0].col << endl;
   cout << "trange row " << trangedat[0].row << endl;
   cout << "trange row " << trangedat[0].year << endl;
   cout << "year " << dyr << endl;
#endif
...
// read in windspeed historical 
   ...
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "ws10 col " << ws10dat[0].col << endl;
   cout << "ws10 row " << ws10dat[0].row << endl;
   cout << "ws10 row " << ws10dat[0].year << endl;
   cout << "year " << dyr << endl;
#endif
   ...
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "vpr col " << vprdat[0].col << endl;
   cout << "vpr row " << vprdat[0].row << endl;
   cout << "vpr row " << vprdat[0].year << endl;
   cout << "year " << dyr << endl;
#endif
    ...
    {
// BSF fix to read one additional value (i.e. transtime + 1)	    
    ...
    else
    {
      ...
      cout << "diag = " << o3dat[0].col << " " << o3dat[0].row << endl;
      if( -1 == gisend )
      {
        cout << "Ran out of Ozone data" << endl << endl;
        flog1 << "Ran out of Ozone data" << endl << endl;
   ...
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "ozone col " << o3dat[0].col << endl;
   cout << "ozone row " << o3dat[0].row << endl;
   cout << "year " << dyr << endl;
#endif
...
//
   ...
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "ndep col " << ndepdat[0].col << endl;
   cout << "ndep row " << ndepdat[0].row << endl;
   cout << "ndep row " << ndepdat[0].year << endl;
   cout << "year " << dyr << endl;
#endif
...
//  Nitrogen Fertilization - Crop
//
//
    if( 1 == telmnt[0].nfert.tnfertcropflag )
   { 
   for( dyr = 0; dyr < (transtime); ++dyr )
   {
   gisend = nfertcropdat[dyr].getdel( ifnfertcrop );
   if( -1 == gisend )
   {
     cout << "Ran out of Nfert_crop data";
     cout << endl << endl;
     flog1 << "Ran out of Nfert_crop data";
     flog1 << endl << endl;

     exit( -1 );
   }

//   cout << "nfertcropdat = " << dyr << " " << nfertcropdat[dyr].col <<  " " << nfertcropdat[dyr].row << " " << nfertcropdat[dyr].adepval << endl;
   }
   }
    else
  {
...
//
   ...
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "nfertcrop col " << nfertcropdat[0].col << endl;
   cout << "nfertcrop row " << nfertcropdat[0].row << endl;
   cout << "nfertcrop row " << nfertcropdat[0].year << endl;
   cout << "year " << dyr << endl;
#endif
...
//  Nitrogen Fertilization - Pasture
//
//
    if( 1 == telmnt[0].nfert.tnfertpastureflag )
   { 
   for( dyr = 0; dyr < (transtime); ++dyr )
   {
   gisend = nfertpasturedat[dyr].getdel( ifnfertpasture );
   if( -1 == gisend )
   {
     cout << "Ran out of Nfert_pasture data";
     cout << endl << endl;
     flog1 << "Ran out of Nfert_pasture data";
     flog1 << endl << endl;

     exit( -1 );
   }

//   cout << "nfertpasturedat = " << dyr << " " << nfertpasturedat[dyr].col <<  " " << nfertpasturedat[dyr].row << " " << nfertpasturedat[dyr].adepval << endl;
   }
   }
    else
  {
...
//
   ...
   cout << "nirr row " << telmnt[0].row << endl;
   cout << "nirr year " << telmnt[0].year << endl;
   cout << "nfertpasture col " << nfertpasturedat[0].col << endl;
   cout << "nfertpasture row " << nfertpasturedat[0].row << endl;
   cout << "nfertpasture row " << nfertpasturedat[0].year << endl;
   cout << "year " << dyr << endl;
#endif
   ...
   fatalerr = telmnt[0].coregerr( flog1,
                                  ...
                                  telmnt[0].row,
                                  "Nfert_pasture",
                                  nfertpasturedat[0].col,
                                  nfertpasturedat[0].row );
...
//    else{ dyr = pdyr; }
  ...
  // BSF fix wind issue from Sonya version
...
//
 ...
 if( 0 == telmnt[0].nfert.tnfertcropflag )
  {
      nfertcropdat[dyr] = nfertcropdat[0];
  }

 if( 0 == telmnt[0].nfert.tnfertpastureflag )
  {
      nfertpasturedat[dyr] = nfertpasturedat[0];
  }
...
//    co2dat[dyr].year = telmnt[0].clm.co2year[pdyr] ;
 ...
 // Assign information from files to telmnt.climate
  ...
  //  Nfert crop
     ...
     telmnt[0].nfertcrop[0][pdyr]= nfertcropdat[pdyr].nfertval;
   //    cout << "ndepval = " << telmnt[0].tem.ndepval << endl;
   //
  //  Nfert pasture
        
   
     telmnt[0].nfertpasture[0][pdyr]= nfertpasturedat[pdyr].nfertval;
   //    cout << "ndepval = " << telmnt[0].tem.ndepval << endl;
   //
...
#ifdef DEBUGX
   cout << "get cohort number data - if part" << endl;
   cout << "mx cohort col " << mxcohrtdat[dyr].col << endl;
   cout << "mx cohort row " << mxcohrtdat[dyr].row << endl;
   cout << "mx cohort contnent " << mxcohrtdat[dyr].contnent << endl;
   cout << "mx cohort year " << mxcohrtdat[dyr].year << endl;
   cout << "year " << dyr << endl;
#endif
        // Check data for spatial coregistration errors
     ...
     #ifdef DEBUGX
   cout << "get cohort number data  - else part" << endl;
#endif
     }
      }
    }
...
//  Pass disturbance information to telmnt[0]
  ...
  for( ichrt = 0; ichrt < telmnt[0].maxcohorts; ++ichrt )
  {
//if(telmnt[0].tem.initFlag == 1) {cout << "lulc clm = " << telmnt[0].col << " " << telmnt[0].row << " " << telmnt[0].year << " " << lulcdat[tstyr][ichrt].year << " " << ichrt+1 << endl;}
