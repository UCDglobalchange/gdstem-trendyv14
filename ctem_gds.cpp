/* **************************************************************
*****************************************************************
CTEM45_GDS.CPP - Calibration version of the Terrestrial Ecosystem
	          Model Version 4.5
...
#include "temconsts_gds.hpp"

//Modules representing climate and TEM

#include "tclm_gds.h"    // Tclm45 Class
#include "tclmdat_gds.h" // Tclmdat45 Class
#include "tsoldat_gds.h" // Soildat45 class
#include "telvdat_gds.h" // Elevdat45 class
#include "tatmdep_gds.h" // Atmospheric deposition class
#include "tnfert_gds.h" // Tnfert class
#include "ttem_gds.h"    // Ttem45 Class
...
//    smoisteq[(xdyr%de_nyears)] = tem.getY(tem.I_SM);

    
    avlneq[(xdyr%de_nyears)] = tem.soil.yravln;
    vegceq[(xdyr%de_nyears)] = tem.veg.yrcarbon;
    soilceq[(xdyr%de_nyears)] = tem.soil.yrorgc;
    vegneq[(xdyr%de_nyears)] = tem.veg.yrnitrogen;
    soilneq[(xdyr%de_nyears)] = tem.soil.yrorgn;
    smoisteq[(xdyr%de_nyears)] = tem.soil.yrsmoist;
...
void initializeCLMGridCell()
{
  ...
  tempfname << nfert.nfertcropfname
            << nfert.nfertcropend;
...
//
  nfert.tnfertcropflag = tem.goxml.getXMLint( tem.gofile, "gofile", "xnfertcropflag" );
  nfert.nfertcropfname = tem.goxml.getXMLstring( tem.gofile, "gofile", "nfertcropfname" );
  nfert.nfertcropend = tem.goxml.getXMLstring( tem.gofile, "gofile", "nfertcropend" );
  if( nfert.tnfertcropflag == 1 ) { flog1 << " transient nfertcrop dataset: " << nfert.nfertcropfname + nfert.nfertcropend << endl; }
  else { flog1 << " long-term average nfertcrop dataset: " << nfert.nfertcropfname << endl; }

//  Input name of Nfert pasture file
//
  nfert.tnfertpastureflag = tem.goxml.getXMLint( tem.gofile, "gofile", "xnfertpastureflag" );
  nfert.nfertpasturefname = tem.goxml.getXMLstring( tem.gofile, "gofile", "nfertpasturefname" );
  nfert.nfertpastureend = tem.goxml.getXMLstring( tem.gofile, "gofile", "nfertpastureend" );
  if( nfert.tnfertpastureflag == 1 ) { flog1 << " transient nfertpasture dataset: " << nfert.nfertpasturefname + nfert.nfertpastureend << endl; }
  else { flog1 << " long-term average nfertpasture dataset: " << nfert.nfertpasturefname << endl; }
...
//  printw( "YRRH   =  %7.2lf | NMIN   =   %6.3lf  | YRNEP   = %6.2lf  | RMROOT  = %6.2lf  | SPERC  = %7.2lf | YRVSM    = %7.2lf || YRFROST  = %5.3lf",
  printw( "YRRH   =  %7.2lf | NMIN   =   %6.3lf  | YRNBP   = %6.2lf  | RMROOT  = %6.2lf  | SPERC  = %7.2lf | YRVSM    = %7.2lf || YRFROST  = %5.3lf",
          tem.microbe.yrrh, tem.microbe.yrnmin, tem.yrnbp, tem.veg.yrrmroot, tem.soil.yrsperc, tem.soil.yrvsm*100.0, tem.ag.yrfrost );
...
//cout << "factor = " << factor << " " << tem.ag.state << endl;
    ...
    case T_FERTFLAG: if( factor != 1 ) { tem.ag.fertflag += 1; }
                         if( tem.ag.fertflag > 1 ) { tem.ag.fertflag = 0; }
                     break;
...
//   cout << "nfertpasturedat = " << dyr << " " << nfertpasturedat[dyr].col <<  " " << nfertpasturedat[dyr].row << " " << nfertpasturedat[dyr].adepval << endl;
 ...
 if( 0 == nfert.tnfertcropflag )
  {
      nfertcropdat[dyr] = nfertcropdat[0];
  }

 if( 0 == nfert.tnfertpastureflag )
  {
...
//  
  ...
  //  Nfert crop
     ...
     nfertcrop[0][pdyr]= nfertcropdat[pdyr].nfertval;
   //    cout << "ndepval = " << telmnt[0].tem.ndepval << endl;
   //
  //  Nfert pasture


     nfertpasture[0][pdyr]= nfertpasturedat[pdyr].nfertval;
   //    cout << "ndepval = " << telmnt[0].tem.ndepval << endl;
   //
